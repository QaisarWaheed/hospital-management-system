...
$select = "SELECT * FROM tokans WHERE 
	`created` <= '$last_date' AND 
	`created` >= '$fro($to_date && $to_date != '0000-00-00' && $to_date != '0000-00-($to_date && $to_date != '0000-00-00' && $to_date != '0000-00-00 00:00:00' ? date_format(date_create($to_date), 'd-m-Y') : ''; ?></span></div>
...
$select = "SELECT * FROM tokans WHERE 
	`created` <= '$last_date' AND 
	`created` >= '$fro($to_date && $to_date != '0000-00-00' && $to_date != '0000-00-00 00:00:00' ? date_format(date_create($to_date), 'd-m-Y') : ''; ?></span></div>
...
{
	while ($row = mysqli_fetch_array($run$to_date), 'd-m-Y') : ''; ?></span></div>
	...
	while ($row = mysqli_fetch_array($run)) 
	{
		$tokan_type_id = $row['tokan_type_id'];
			$select_count = "SELECT * FROM tokans WHERE 
				user_id = '$u_id' AND 
				`created` <=($token_date && $token_date != '0000-00-00' && $token_date != '0000-00-00 00:00:00' ? date_format(date_create($token_date), "h:i A") : ''.'</td>
			<td>'.date_format(date_create($token_date), "d M").'</td>
			<td style="text-align: right;">'.$row['id'].'</td>
			<td>'.$name.'('.$genders.')</td>
			<td style="text-align: right;">'.$age.'</td>
...
if (mysqli_num_rows($run) > 0) 
{
	while ($row = mysqli_fetch_array($run)) 
	{
		$tokan_type_id = $row['tokan_type_id'];
			$select_count = "SELECT * FROM tokans WHERE 
				user_id = '$u_id' AND 
				`crea($token_date && $token_date != '0000-00-00' && $token_date != '0000-00-00 00:00:00' ? date_format(date_create($token_date), "h:i A") : ''.'</td>
			<td>'.date_format(date_create($token_date), "d M").'</td>
			<td style="text-align: right;">'.$row['id'].'</td>
			<td>'.$name.'('.$genders.')</td>
			<td style="text-align: right;">'.$age.'</td>
	...
	while ($row = mysqli_fetch_array($run)) 
	{
		$tokan_type_id = $row['tokan_type_id'];
			$select_count = "SELECT * FROM tokans WHERE 
				user_id = '$u_id' AND 
				`created` <= '$last_date' AND 
				`created` >= '$from_date' AND tokan_type_id = '$tokan_type_id' AND `status$token_date), "h:i A") : ''.'</td>
			<td>'.date_format(date_create($token_date), "d M").'</td>
			<td style="text-align: right;">'.$row['id'].'</td>
			<td>'.$name.'('.$genders.')</td>
			<td style="text-align: right;">'.$age.'</td>
...
if (mysqli_num_rows($run) > 0) 
{
	while ($row = mysqli_fetch_array($run)) 
	{
		$tokan_type_id = $row['tokan_type_id'];
			$select_count = "SELECT * FROM tokans WHERE 
				`created` <= '$last_date' AND 
				`created` >= '$from_($token_date && $token_date != '0000-00-00' && $token_date != '0000-00-00 00:00:00' ? date_format(date_create($token_date), "d M") : ''.'</td>
			<td style="text-align: right;">'.$row['id'].'</td>
			<td>'.$name.'('.$genders.')</td>
			<td style="text-align: right;">'.$age.'</td>
			<td>'.$pre.'</td>
