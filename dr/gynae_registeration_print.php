			        ...
			        <tbody>
			         ...
			         if(mysqli_num_rows($run) > 0)
			         {
			             while($row = mysqli_fetch_array($run))
			             {
			                 ...
			                 $phone = $row['phone'];
			                 $gravide = $row['gravide'];
			                 $start_date = ($row['weeks'] && $row['weeks'] != '0000-00-00' && $row['weeks'] != '0000-00-00 00:00:00' ? date_format(date_create($row['weeks']), 'd/m/Y H:i:s') : '';
...
<?php mysqli_close($con); ?>($row['weeks'] && $row['weeks'] != '0000-00-00' && $row['weeks'] != '0000-00-00 00:00:00' ? date_format(date_create($($row['weeks'] && $row['weeks'] != '0000-00-00' && $row['weeks'] != '0000-00-00 00:00:00' ? date_format(date_create($row['weeks']), 'd/m/Y') : '';
			                 $next_visit_date = $row['next_visit_date'];
                             $to_date = date('d/m/Y H:i:s');
                                $datefrom = DateTime::createFromFormat('d/m/Y H:i:s',$start_date);
                                $dateto = DateTime::createFromFormat('d/m/Y H:i:s',$to_date);
...
<?php mysqli_close($con); ?>($row['weeks'] && $row['weeks'] != '0000-00-00' && $row['weeks'] != '0000-00-00 00:00:00' ? date_format(date_create($row['weeks']), 'd/m/Y') : '';
			                 $next_visit_date = $row['next_visit_date'];
                             $to_date = date('d/m/Y H:i:s');
                                $datefrom = DateTime::createFromFormat('d/m/Y H:i:s',$start_date);
                                $dateto = DateTime::createFromFormat('d/m/Y H:i:s',$to_date);
...
<?php mysqli_close($con); ?>$row['weeks']), 'd/m/Y') : '';
			                 $next_visit_date = $row['next_visit_date'];
                             $to_date = date('d/m/Y H:i:s');
                                $datefrom = DateTime::createFromFormat('d/m/Y H:i:s',$start_date);
                                $dateto = DateTime::createFromFormat('d/m/Y H:i:s',$to_date);
