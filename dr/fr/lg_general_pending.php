...
if(isset($_GET['from_date']) && $_GET['from_date'] != '')
{
    if($_GET['from_date'] != '' && $_GET['to_date'] != '')
    {
        ...
        $to_date .= " 23:59:59";
        $select = "SELECT * FROM `branch_daily_pending_details` WHERE token_no IN (SELECT id FROM tokans WHERE status = '1') AND created >= '$from_date' AND created <= '$to_date' ";
    }
    else
    {
...
{
    $br_id = $branch_id;
    $select = "SELECT * FROM `branch_daily_pending_details` WHERE token_no IN (SELECT id FROM tokans WHERE status = '1' ) ";
}
$s = 0 ;
...
<?php
    while($row = mysqli_fetch_array($run))
    {
        ...
        $get_branch = get_branch_tag_by_token_no($token_no);
