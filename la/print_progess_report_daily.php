...
<html>
<head>
    <title><?php echo htmlspecialchars(get_branch_tag_by($br_id), ENT_QUOTES, 'UTF-8'); ?> <?php echo ($date && $date != '0000-00-00' && $date != '0000-00-00 00:00:00' ? date_format(date_create($date), 'd-M-Y') : ''; ?> DAILY PROGRESS REPORT</title>
</head>
<body>
...
<?php
mysqli_close($con);
($date && $date != '0000-00-00' && $date != '0000-00-00 00:00:00' ? date_format(date_create($date), 'd-M-Y') : ''; ?></h3>
</caption>
    <thead>
        <tr>
            <th>S#</th>
...
if ($run && mysqli_num_rows($run) > 0) {
    while ($row = mysqli_fetch_array($run)) {
        ...
        $pct = ($opd > 0 && $labs > 0) ? (int) (($labs / $opd) * 100) : 0;
        if ($opd > 0 && $labs >= $opd) {
            $pct ($date && $date != '0000-00-00' && $date != '0000-00-00 00:00:00' ? date_format(date_create($date), 'd-M-Y') : ''; ?></h3>
</caption>
    <thead>
        <tr>
            <th>S#</th>
