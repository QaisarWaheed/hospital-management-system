...
<caption>
    ...
    if(mysqli_num_rows($run) > 0)
    {
        ...
        // $select_data = "SELECT item_by_doctor.category_id, COUNT(item_by_doctor.category_id) AS count_data, COUNT(DISTINCT item_by_doctor.tokan_no) AS count_token, SUM(item_by_doctor.sale_price)AS lab_cash FROM `item_by_doctor` WHERE item_by_doctor.created LIKE '$date%' AND item_by_doctor.doctor_id = '$doctor_id' AND item_by_doctor.branch_id = '$br_id' AND item_by_doctor.category_id IN( 2, 3, 29, 31, 32, 33, 34, 36, 37, 38, 39, 40, 41, 42, 44 ) GROUP BY item_by_doctor.category_id;";        
        // $select_data = "SELECT items.category_id, COUNT(items.category_id) AS count_data, COUNT(DISTINCT item_by_doctor.tokan_no) AS count_token, SUM(DISTINCT tokans.cash) AS count_cash  FROM `item_by_doctor` INNER JOIN item_register_to_branches ON item_by_doctor.item_id = item_register_to_branches.id INNER JOIN items ON item_register_to_branches.item_id = items.id INNER JOIN tokans ON item_by_doctor.tokan_no = tokans.id  WHERE tokans.created LIKE '$date%' AND item_by_doctor.doctor_id = '$doctor_id' AND item_by_doctor.branch_id = '$br_id' AND items.category_id IN (2, 3, 29, 31, 32, 33, 34, 36, 37, 38, 39, 40, 41, 42, 44) AND tokans.status = '1' GROUP BY items.category_id ";
        $select_data = "SELECT
...
FROM `item_by_doctor` WHERE item_by_doctor.created LIKE '$date%' AND item_by_doctor.doctor_id = '$doctor_id' AND item_by_doctor.branch_id = '$br_id' AND item_by_doctor.category_id IN(2, 3, 29, 31, 32, 33, 34, 36, 37, 38, 39, 40, 41, 42, 44); ";
        ...
        <tr>
            ...
            <td><?php echo number_format($lab_test_cash); ?></td>
            <td><?php echo $usgs; ?></td>
            <td><?php echo $svds; ?></td>
            <td><?php echo $dncs; ?></td>
            ...
            <td style = "text-align: right;"><?php echo number_format($cash); ?></td>
        </tr>
        <?php 
        $tests = 0;
        $lab_test_cash = 0;
