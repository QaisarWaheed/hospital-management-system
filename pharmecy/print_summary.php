...
<div style="text-align: center;">
	...
	<div>
		<div style="float: left;">
				...
				<th>Date</th>
				<th style="text-align: left;"><?php echo ($from_date && $from_date != '0000-00-00' && $from_date != '0000-00-00 00:00:00' ? date_format(date_create($from_date), 'd-m-Y') : ''; ?> To <?php echo date_format(date_create($to_date), 'd-m-Y'); ?></th><br>
				<th>User Name:</th>
...
$select = "SELECT * FROM tokans WHERE 
	...
	`stat($to_date && $to_date != '0000-00-00' && $to_date != '0000-00-($to_date && $to_date != '0000-00-00' && $to_date != '0000-00-00 00:00:00' ? date_format(date_create($to_date), 'd-m-Y') : ''; ?></th><br>
				<th>User Name:</th>
				<th style="text-align: left;"><?php echo $user_name; ?></th>
		</div>
		<div style="float: right;">
...
$select = "SELECT * FROM tokans WHERE 
	...
	`created` <= '$last_date' AND 
	`created` >= '$from_date' AND
	`stat($to_date && $to_date != '0000-00-00' && $to_date != '0000-00-00 00:00:00' ? date_format(date_create($to_date), 'd-m-Y') : ''; ?></th><br>
				<th>User Name:</th>
				<th style="text-align: left;"><?php echo $user_name; ?></th>
		</div>
		<div style="float: right;">
	...
	while ($row = mysqli_fetch_array($run)) 
	{
		$tokan_type_id = $row['tokan_type_id'];
			$select_count = "SELECT * FROM tokans WHERE 
				user_id = '$user_id' AND 
				`created` <= '$last_date' AND 
				`created` >= '$($token_date && $token_date != '0000-00-00' && $token_date != '0000-00-00 00:00:00' ? date_format(date_create($token_date), "d-m-y") : ''.'</td>
			<td style="text-align: right;">'.$row['id'].'</td>
			<td>'.$name.'</td>
			<td style="text-align: right;">'.$age.'</td>
			<td>'.$genders.'</td>
