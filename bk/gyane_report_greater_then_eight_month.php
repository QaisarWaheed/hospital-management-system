...
$four_month_age_date = date('Y-m-d', strtotime('+4 month', strtotime(date('Y-m-d'))));
if(isset($_GET['br_id']) && $_GET['br_id'] != '')
{
    $br_id = $_GET['br_id'];
    $select_dr = "SELECT  gynae_register.id, gynae_register.token_no, gynae_register.next_visit_date, gynae_register.weeks, patients.id, patients.name, gynae_register.phone, gynae_register.created, branchs.tag_name, users.u_name, COUNT(gynae_register_history.id) AS total_visits FROM `gynae_register` INNER JOIN users ON register_by_doctor = users.id INNER JOIN branchs ON gynae_register.branch_id = branchs.id LEFT JOIN gynae_register_history ON gynae_register.id = gynae_register_history.gynae_register_id INNER JOIN tokans ON gynae_register.token_no = tokans.id INNER JOIN patients ON tokans.patient_id = patients.id WHERE gynae_register.branch_id = '$br_id' gynae_register.weeks > '$four_month_age_date' AND gynae_register.weeks <= '$today_date' AND gynae_register.status = '1' GROUP BY gynae_register.id ORDER BY gynae_register.weeks  ";
}
else
{
...
<caption style = "text-align: center; caption-side: top; color: black;">
    ...
    <p>8 MONTH TIME IS COMPLETED. NEDD TO CONTACT</p>
</caption>
    <thead>
        <tr>
            <th>S#</th>
