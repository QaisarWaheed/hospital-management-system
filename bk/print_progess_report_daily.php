<?php 
include 'includes/connect.php'; 
if(isset($_GET['date']))
{
    $date = $_GET['date'];
    $br_id = $_GET['br_id'];
}
elseif(isset($_POST['date']))
{
    $date = $_POST['date'];
    $br_id = $_POST['br_id'];
}
else
{
    exit(0);
}
?>
<html>
<head>
    <style>
        @media print {
           .noprint {
              visibility: hidden;
           }
        }
    </style>
    <title><?php echo date_format(date_create($date), "d-m-Y").' '.get_branch_tag_by($br_id); ?> PROGRESS REPORT</title>
</head>
<body>
<div>
    <a class = "noprint" href = "dashboard.php">GOTO DASHBOARD</a>
</div>
<table border = "solid">
<caption>
    <h2><?php echo $company_name; ?></h2>
    <h2><?php echo get_branch_name_by($br_id); ?></h2>
    <h3>PROGRESS DATE <?php echo date_format(date_create($date), " d F Y"); ?></h3>
    <h5 align = "center">PROGRESS REPORT IS GENERATED BY: <i><?php echo $bk_name; ?></i>, AT: <i><?php echo date('H:i:s d-m-Y'); ?></i></h5>
</caption>
    <thead>
        <tr>
            <th>S#</th>
            <th>ID</th>
            <th>NAME</th>
            <th>OPD</th>
            <th>CONS</th>
            <th>LAB</th>
            <th>CASH</th>
            <th>USG</th>
            <th>SVD</th>
            <th>D&C</th>
            <th>OPERATION</th>
            <th>DENTAL</th>
            <th>SKIN</th>
            <th>EYE</th>
            <th>ADMISSION</th>
            <th>EMERGENCY</th>
            <th>ECG</th>
            <th>GYNAE TOKEN</th>
            <th>GYNAE SYSTEM</th>
            <th>REFERREL BY</th>
            <th>REFERREL TO</th>
            <th>COLLECTION</th>
        </tr>
    </thead>
<?php
    $s = 0; 
    $lab_cash = 0;
    $collections = 0;
    $opds = 0;
    $cons_opds = 0;
    $labs = 0;
    $usgs = 0;
    $svds = 0;
    $dncs = 0;
    $procedures = 0;
    $cons_dentals = 0;
    $skins = 0;
    $eyes = 0;
    $admissions = 0;
    $emergency = 0;
    $ecg = 0;
    $gynaes = 0;
    $gynae_system = 0;
    $refered = 0;
    $refered_to = 0;
    $lab_cash = 0;
    $total_lab_cash = 0;

    $total_cash = 0;
    $total_collections = 0;
    $total_admissions = 0;
    $total_procedures = 0;
    $total_gynae_system = 0;
    $total_dncs = 0;
    $total_svds = 0;
    $total_skins = 0;
    $total_eyes = 0;
    $total_usg = 0;
    $total_lab = 0;
    $total_medicine = 0;
    $total_opds = 0;
    $total_cons_opds = 0;
    $total_gynaes = 0;
    $total_cons_dentals = 0;
    $total_ecg = 0;
    
$select = "SELECT DISTINCT `doctor_id` FROM `tokans` WHERE doctor_id IN (SELECT `id` FROM `users` WHERE `branch_id` = '$br_id') AND created like '$date%' AND `branch_id` = '$br_id' ORDER BY `doctor_id` ";
$run = mysqli_query($con, $select);
if(mysqli_num_rows($run) > 0)
{
    echo '<tbody>';
    while($row = mysqli_fetch_array($run))
    {
        $s = $s + 1;
        $doctor = $row['doctor_id'];

        $opds = mysqli_num_rows(mysqli_query($con, "SELECT id FROM tokans WHERE `tokan_type_id` < 9 AND status = 1 and doctor_id = '$doctor' AND created like '$date%' AND `branch_id` = '$br_id' "));
        $total_opds = $total_opds + $opds;
        
        $collection = mysqli_query($con, "SELECT SUM(cash) FROM tokans WHERE status = 1 and doctor_id = '$doctor' AND created like '$date%' AND `branch_id` = '$br_id' ");
        if(mysqli_num_rows($collection) == 1)
        {
            while($row_collection = mysqli_fetch_array($collection))
            {
                $collections = $row_collection['0'];
            }
        }
        else
        {
            $collections = 0;
        }
        $total_collections = $total_collections + $collections;

        $select_data = "SELECT item_by_doctor.category_id, COUNT(item_by_doctor.category_id) AS count_data, COUNT(DISTINCT item_by_doctor.tokan_no) AS count_token, 
SUM(
    CASE
    	WHEN (`fix_dose` = 0 AND `tokan_type_id` = 102) THEN (`dose`*`feed`*`days`)*`sale_price_poor` 
    	WHEN (`fix_dose` = 0 AND `tokan_type_id` = 103) THEN (`dose`*`feed`*`days`)*`sale_price_member` 
    	WHEN (`fix_dose` = 0 AND `tokan_type_id` = 104) THEN (`dose`*`feed`*`days`)*`sale_price_general`
    	WHEN (`fix_dose` > 0 AND `tokan_type_id` = 102) THEN (`fix_dose`)*`sale_price_poor` 
    	WHEN (`fix_dose` > 0 AND `tokan_type_id` = 103) THEN (`fix_dose`)*`sale_price_member` 
    	WHEN (`fix_dose` > 0 AND `tokan_type_id` = 104) THEN (`fix_dose`)*`sale_price_general`
    END)
AS total_cash 
FROM `item_by_doctor` WHERE item_by_doctor.created LIKE '$date%' AND item_by_doctor.doctor_id = '$doctor' AND item_by_doctor.branch_id = '$br_id' AND item_by_doctor.category_id IN( 2, 3, 29, 31, 32, 33, 34, 36, 37, 38, 39, 40, 41, 42, 44 ) GROUP BY item_by_doctor.category_id;";
        // $select_data = "SELECT items.category_id, COUNT(items.category_id) AS count_data, COUNT(DISTINCT item_by_doctor.tokan_no) AS count_token FROM `item_by_doctor` INNER JOIN item_register_to_branches ON item_by_doctor.item_id = item_register_to_branches.id INNER JOIN items ON item_register_to_branches.item_id = items.id INNER JOIN tokans ON item_by_doctor.tokan_no = tokans.id  WHERE tokans.created LIKE '$date%' AND item_by_doctor.doctor_id = '$doctor' AND item_by_doctor.branch_id = '$br_id' AND items.category_id IN (2, 3, 29, 31, 32, 33, 34, 36, 37, 38, 39, 40, 41, 42, 44) AND tokans.status = '1' GROUP BY items.category_id ";
        $run_data = mysqli_query($con, $select_data);
        if(mysqli_num_rows($run_data) > 0)
        {
            while($row_data = mysqli_fetch_array($run_data))
            {
                $category_id = $row_data['category_id'];
                $count_data = $row_data['count_data'];
                $count_token = $row_data['count_token'];
                $total_cash = $row_data['total_cash'];
                if($category_id == 2)
                {
                    $labs = $count_token;
                    $total_lab = $total_lab + $labs;
                    $lab_cash = $total_cash;
                    $total_lab_cash = $total_cash + $total_lab_cash;
                }
                elseif($category_id == 3)
                {
                    $procedures = $count_token;
                    $total_procedures = $total_procedures + $procedures;
                }
                elseif($category_id == 29)
                {
                    $cons_opds = $count_token;
                    $total_cons_opds = $total_cons_opds + $cons_opds;
                }
                elseif($category_id == 31)
                {
                    $cons_dentals = $count_token;
                    $total_cons_dentals = $total_cons_dentals + $cons_dentals;
                }
                elseif($category_id == 32)
                {
                    $cons_skins = $count_token;
                    $total_cons_skins = $total_cons_skins + $cons_skins;
                }
                elseif($category_id == 33)
                {
                    $cons_eyes = $count_token;
                    $total_cons_eyes = $total_cons_eyes + $cons_eyes;
                }
                elseif($category_id == 37)
                {
                    $svds = $count_token;
                    $total_svds = $total_svds + $svds;
                }
                elseif($category_id == 38)
                {
                    $dncs = $count_token;
                    $total_dncs = $total_dncs + $dncs;
                }
                elseif($category_id == 39)
                {
                    $usgs = $count_token;
                    $total_usgs = $total_usgs + $usgs;
                }
                elseif($category_id == 40)
                {
                    $admissions = $count_token;
                    $total_admissions = $total_admissions + $admissions;
                }
                elseif($category_id == 41)
                {
                    $gynaes = $count_token;
                    $total_gynaes = $total_gynaes + $gynaes;
                }
                elseif($category_id == 42)
                {
                    $emergency = $count_token;
                    $total_emergency = $total_emergency + $emergency;
                }
                elseif($category_id == 44)
                {
                    $ecg = $count_token;
                    $total_ecg = $total_ecg + $ecg;
                }
            }
        }

        $gynae_system = mysqli_num_rows(mysqli_query($con, "SELECT * FROM `gynae_register` WHERE doctor_id = '$doctor' AND created like '$date%' AND branch_id = '$br_id'"));
        $total_gynae_system = $total_gynae_system + $gynae_system;
        
        $refered = mysqli_num_rows(mysqli_query($con, "SELECT * FROM `referral_patients` WHERE `from_user_id` = '$doctor' AND `referral_patient_status` > '1' AND `referral_patient_created` like '$date%' "));
        $total_refered = $total_refered + $refered;
        
        $refered_to = mysqli_num_rows(mysqli_query($con, "SELECT * FROM `referral_patients` WHERE `to_user_id` = '$doctor' AND `referral_patient_status` > '1' AND `referral_patient_created` like '$date%' "));
        $total_refered_to = $total_refered_to + $refered_to;

        $doctor_name = get_uname_by_id($doctor);
        echo ' <tr style = "text-align: right;">
                <td>'.$s.'</td>
                <td>'.$doctor.'</td>
                <td style = "text-align: left;">'.$doctor_name.'</td>
                <td>'.$opds.'</td>
                <td>'.$cons_opds.'</td>
                <td>'.$labs.'</td>
                <td>'.number_format($lab_cash).'</td>';
                echo '<td>'.$usgs.'</td>
                <td>'.$svds.'</td>
                <td>'.$dncs.'</td>';
                echo '<td>'.$procedures.'</td>
                <td>'.$cons_dentals.'</td>
                <td>'.$skins.'</td>
                <td>'.$eyes.'</td>
                <td>'.$admissions.'</td>
                <td>'.$emergency.'</td>
                <td>'.$ecg.'</td>
                <td>'.$gynaes.'</td>
                <td>'.$gynae_system.'</td>
                <td>'.$refered.'</td>
                <td>'.$refered_to.'</td>
                <td>'.number_format($collections).'</td>
            </tr>';
        $opds = 0;
        $lab_cash = 0;
        $cons_opds = 0;
        $labs = 0;
        $usgs = 0;
        $svds = 0;
        $dncs = 0;
        $procedures = 0;
        $cons_dentals = 0;
        $skins = 0;
        $eyes = 0;
        $admissions = 0;
        $emergency = 0;
        $ecg = 0;
        $gynaes = 0;
        $gynae_system = 0;
        $refered = 0;
        $refered_to = 0;
        $collections = 0;
    }
    echo '</tbody>';
    echo '<tfoot>
            <tr style = "text-align: right;">
                <th colspan = "3"></th>
                <th>'.$total_opds.'</th>
                <th>'.$total_cons_opds.'</th>
                <th>'.$total_lab.'</th>
                <th>'.number_format($total_lab_cash).'</th>';
                echo '<th>'.$total_usgs.'</th>
                <th>'.$total_svds.'</th></th>
                <th>'.$total_dncs.'</th>';
                echo '<th>'.$total_procedures.'</th>
                <th>'.$total_cons_dentals.'</th>
                <th>'.$total_skins.'</th>
                <th>'.$total_eyes.'</th>
                <th>'.$total_admissions.'</th>
                <th>'.$total_emergency.'</th>
                <th>'.$total_ecg.'</th>
                <th>'.$total_gynaes.'</th>
                <td>'.$total_gynae_system.'</th>
                <td>'.$total_refered.'</th>
                <td>'.$total_refered_to.'</th>
                <td>'.number_format($total_collections).'</th>
            </tr>
        </tfoor>';
}
?>
</table>
</body>
</html>